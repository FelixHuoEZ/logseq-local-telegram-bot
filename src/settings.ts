...
class Settings {
  constructor(onUpdate: (key: string) => void) {
    ...
    logseq.onSettingsChanged((new_settings, old_settings) => {
      ...
      if (
        new_settings.scheduledNotificationTime !=
        old_settings.scheduledNotificationTime
      ) {
        onUpdate(nameof<Settings>("scheduledNotificationTime"));
      }

      if (
        new_settings.deadlineNotificationTime !=
        old_settings.deadlineNotificationTime
      ) {
        onUpdate(nameof<Settings>("deadlineNotificationTime"));
      }

      if (
        new_settings.enableCustomizedCommand !=
        old_settings.enableCustomizedCommand
      ) {
        onUpdate(nameof<Settings>("enableCustomizedCommand"));
      }

      if (new_settings.enablePageLogging != old_settings.enablePageLogging) {
        onUpdate(nameof<Settings>("enablePageLogging"));
      }
    });
  }
  ...
  public get authorizedUsers(): string[] {
    return logseq
      .settings!.authorizedUsers.split(",")
  ...
  public get enablePageLogging(): boolean {
    return logseq.settings!.enablePageLogging;
  }

  public get enableCustomizedCommandFromMessage(): boolean {
    return (
      this.enableCustomizedCommand &&
      logseq.settings!.enableCustomizedCommandFromMessage
    );
  }
...
const settingsSchema: SettingSchemaDesc[] = [
  ...
  },
  {
    key: "isMainBot",
    description:
      "If you have multiple Logseq open at the same time, probably from different devices, only one should be set to true, to avoid conflicts of multiple bots running together",
    type: "boolean",
    default: false,
    title: "Is Main Bot",
  },
  {
    key: "authorizedUsers",
    description:
      'Be sure to add your username in authorizedUsers, because your recently created bot is publicly findable and other peoples may send messages to your bot. If there are multiple usernames, separate them by ",", with optional leading or trailing space, like "your_username1, your_username2". If you leave this empty - all messages from all users will be processed!',
    type: "string",
    default: "",
  ...
  },
  {
    key: "pageName",
    description:
      "The name of the page that all regular messages from Telegram are added to. \"Journal\" is reserved for today's Journal, and it's the default. The page should be available.",
    type: "string",
    default: "Journal",
    title: "Page Name",
  },
  {
    key: "inboxName",
    description:
      "The content of the block that all regular messages from Telegram are added to. If it's not available, a new Inbox will be created at the end of target page. If its value is empty, the messages will be added to the target page",
    type: "string",
    default: "#Inbox",
    title: "Inbox Name",
  },
  {
    key: "appendAtBottom",
    description:
      "If it's set to true, the new messages will be appended at the bottom of Inbox, instead of the top.",
    type: "boolean",
    default: false,
  ...
  },
  {
    key: "addTimestamp",
    description:
      "If it's set to true, message received time in format HH:mm will be added to the front of message text",
    type: "boolean",
    default: false,
    title: "Add Timestamp",
  },
  {
    ...
    description:
      "The local time of notificaiton for not-done task with scheduled date. The message should be sent one day before the scheduled at this specific time. Clearing it disable this feature. It's only enabled for main bot",
    type: "string",
    default: "",
  ...
  },
  {
    ...
    description:
      "The local time of notificaiton for not-done task with deadline date. The message should be sent one day before the deadline at this specific time. Clearing it disables this feature. It's only enabled for main bot",
    type: "string",
    default: "",
  ...
  },
  {
    ...
    description:
      "Whether to enable customized command mode, which enables eligible users to run native js/ts or query datascript. **This is still experimenting**",
    type: "boolean",
    default: false,
  ...
  },
  {
    ...
    description:
      "Whether to allow messages to include customized command, which enables eligible users to add new commands from Telegram. **This is still experimenting**",
    type: "boolean",
    default: false,
    title: "Enable Customized Command From Message",
  },
  {
    key: "enablePageLogging",
    description:
      "Whether to append logs to the Local Telegram Bot Log page. Turn this on when debugging.",
    type: "boolean",
    default: false,
    title: "Enable Page Logging",
  },
];
